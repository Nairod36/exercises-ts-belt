...
export const ArrayFn11 = () => {
  const slice = A.slice(products, 1, 3);

  const takeExactly = A.takeExactly(products, 2);

  const takeWhile = A.takeWhile(products, (product) => product.price < 100);

  console.log(slice, takeExactly, takeWhile);
};
